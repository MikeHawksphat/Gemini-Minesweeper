...
function broadcast(data) {
    if (!isHost) return;
    console.log('Broadcasting to:', Object.values(connections).map(c => c.peer));
...
function init(broadcastUpdate) {
    ...
    isFirstClick = true;
    flagsPlaced = 0;
    board = Array.from({ length: ROWS }, () => Array(COLS).fill(null).map(() => ({ mine: false, revealed: false, flagged: false, count: 0 })));
    minesLocation = [];
    timer = 0;
    gameOver = false;
...
function getState() {
    return { rows: ROWS, cols: COLS, mines: MINES, board: board, flagsPlaced: flagsPlaced, gameOver: gameOver, timer: timer, isFirstClick: isFirstClick };
}
...
function placeMines(firstClickRow, firstClickCol) {
    const excludedCells = new Set(getAdjacentCells(firstClickRow, firstClickCol).map(c => `${c.row}-${c.col}`));
    ...
    while (changed) {
        ...
        for (let r = 0; r < ROWS; r++) {
            for (let c = 0; c < COLS; c++) {
                ...
                if (cell.revealed && cell.count > 0) { // Only apply rules to revealed numbered cells
                    ...
                    if (cell.count - flaggedNeighborsCount === unrevealedUnflaggedNeighbors.length && unrevealedUnflaggedNeighbors.length > 0) {
                        unrevealedUnflaggedNeighbors.forEach(n => {
                            const neighbor = simulationBoard[n.row][n.col];
                            if (!neighbor.flagged) { // Only flag if not already flagged
                    ...
                    if (cell.count === flaggedNeighborsCount && unrevealedUnflaggedNeighbors.length > 0) {
                        unrevealedUnflaggedNeighbors.forEach(n => {
                            const neighbor = simulationBoard[n.row][n.col];
                            if (!neighbor.revealed && !neighbor.flagged) { // Only reveal if not already revealed or flagged
        ...
        for (let r1 = 0; r1 < ROWS; r1++) {
            for (let c1 = 0; c1 < COLS; c1++) {
                ...
                if (cell1.revealed && cell1.count > 0) {
                    ...
                    const unrevealedUnflagged1 = neighbors1.filter(n => !simulationBoard[n.row][n.col].revealed && !simulationBoard[n.row][n.col].flagged);
                    const flaggedCount1 = neighbors1.filter(n => simulationBoard[n.row][n.col].flagged).length;

                    for (const { row: r2, col: c2 } of neighbors1) { // Check adjacent revealed cells
                        ...
                        if (cell2.revealed && cell2.count > 0 && !(r1 === r2 && c1 === c2)) {
                            ...
                            const unrevealedUnflagged2 = neighbors2.filter(n => !simulationBoard[n.row][n.col].revealed && !simulationBoard[n.row][n.col].flagged);
                            const flaggedCount2 = neighbors2.filter(n => simulationBoard[n.row][n.col].flagged).length;
